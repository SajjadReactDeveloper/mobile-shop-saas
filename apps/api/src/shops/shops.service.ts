...
export class ShopsService {
  ...
  findById(id: string) {
    return this.prisma.shop.findUniqueOrThrow({ where: { id }, include: { subscription: true } })
  }

  update(id: string, data: { name?: string; city?: string; address?: string; phone?: string; enabledModules?: string[] }) {
  ...
  async getStats(shopId: string) {
    const [products, customers, pendingRepairs] = await Promise.all([
      ...
      this.prisma.repairJob.count({ where: { shopId, status: { notIn: ['DELIVERED', 'CANCELLED'] } } }),
    ])
