...
export class InventoryService {
  ...
  findAll(shopId: string, category?: ProductCategory, lowStock?: boolean) {
    return this.prisma.product.findMany({
      where: {
        ...
        isActive: true,
        ...(category && { category }),
        ...(lowStock && { stockQty: { lte: this.prisma.product.fields.lowStockAlert } }),
      },
  ...
  async getLowStock(shopId: string) {
    const products = await this.prisma.product.findMany({ where: { shopId, isActive: true } })
  ...
  async addStock(productId: string, shopId: string, dto: AddStockDto) {
    const product = await this.prisma.product.findUniqueOrThrow({ where: { id: productId, shopId } })

    if (product.imeiTracked) {
      if (!dto.imeis || dto.imeis.length !== dto.qty) {
        throw new BadRequestException(`Must provide exactly ${dto.qty} IMEI numbers for this product`)
      }

      const duplicate = await this.prisma.imeiLog.findFirst({
        where: { productId, imei: { in: dto.imeis } },
      })
      if (duplicate) throw new BadRequestException(`IMEI ${duplicate.imei} already registered`)
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (product.imeiTracked && dto.imeis) {
        await tx.imeiLog.createMany({
          data: dto.imeis.map((imei) => ({ productId, imei, status: 'IN_STOCK' })),
        })
      }
  ...
  softDelete(id: string, shopId: string) {
    return this.prisma.product.update({ where: { id, shopId }, data: { isActive: false } })
  }
}
