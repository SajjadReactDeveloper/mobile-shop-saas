...
export class EasyLoadController {
  ...
  load(@CurrentUser() user: any, @Param('id') id: string, @Body() body: any) {
    return this.easyLoadService.load(id, user.shopId, body)
  }

  @Post('accounts/:id/topup')
  topup(@CurrentUser() user: any, @Param('id') id: string, @Body() body: any) {
    return this.easyLoadService.topup(id, user.shopId, body)
  }

  @Get('daily-summary')
  getDailySummary(@CurrentUser() user: any, @Query('date') date: string) {
    return this.easyLoadService.dailySummary(user.shopId, date ?? new Date().toISOString().split('T')[0])
  }
}
